...
public class RentACarServisService {
	...
	public RentACarServis dodajRentACarServis(RentACarServis noviServis) throws NevalidniPodaciException {
		...
		RentACarServis rac = new RentACarServis(noviServis.getNaziv(), noviServis.getPromotivniOpis(),
				noviServis.getAdresa());
	...
	public Collection<VoziloDTO> vratiVozilaOsnovno(Collection<Vozilo> vozila) {
		...
		for (Vozilo vozilo : vozila) {
			...
			VoziloDTO vozDto = new VoziloDTO(vozilo.getNaziv(), vozilo.getMarka(), vozilo.getModel(),
					vozilo.getGodina_proizvodnje(), vozilo.getBroj_sjedista(), vozilo.getTip_vozila(),
					vozilo.getBroj_vrata(), vozilo.getKilovati(), vozilo.getCijena_po_danu(), adresa, vozilo.getId());
	...
	public Collection<Poslovnica> pretraziRac(PretragaRacDTO kriterijumiPretrage) throws NevalidniPodaciException {
		...
		while (it.hasNext()) {
			...
			if (!tekuciRac.getAdresa().getPunaAdresa().toUpperCase()
					.contains(kriterijumiPretrage.getNazivDestinacije().toUpperCase())) {
				it.remove();
			} else if (!tekuciRac.getNaziv().toUpperCase()
					.contains(kriterijumiPretrage.getNazivRacServisa().toUpperCase())) {
				it.remove();
			}
		...
		for (Vozilo voz : rac.getVozila()) {
			if (datumNull == true) {
				...
				for (RezervacijaVozila r : this.rezervacijaVozilaRepository.findAllByRezervisanoVozilo(voz)) {
					...
					if (pocetniDatum.compareTo(r.getDatumVracanjaVozila()) < 0
							&& krajnjiDatum.compareTo(r.getDatumPreuzimanjaVozila()) > 0) {
						dodajVozilo = false;
						break;
					} else {
						dodajVozilo = true;
	...
	public Collection<Vozilo> pretraziVozilaServisa(PretragaVozilaDTO kriterijumiPretrage)
		...
		if (pocetniDatum.compareTo(krajnjiDatum) == 0) {
			throw new NevalidniPodaciException(
					"Između datuma preuzimanja i datuma vraćanja vozila mora biti barem jedan dan razlike.");
		}
		...
		for (Vozilo vozilo : slobodnaVozila) {
			...
			if (vozilo.getTip_vozila().equalsIgnoreCase(kriterijumiPretrage.getTipVozila())
				...
				if (kriterijumiPretrage.getMinimalnaCijenaPoDanu() != null
						&& kriterijumiPretrage.getMaksimalnaCijenaPoDanu() != null) {
					if (vozilo.getCijena_po_danu() >= kriterijumiPretrage.getMinimalnaCijenaPoDanu()
							&& vozilo.getCijena_po_danu() <= kriterijumiPretrage.getMaksimalnaCijenaPoDanu()) {
	...
	public String rezervisiVozilo(RezervacijaVozilaDTO rezervacijaDTO, Long idServisa) throws NevalidniPodaciException {
		...
		if (!putovanjeSearch.isPresent()) {
			throw new NevalidniPodaciException("Ne postoji putovanje sa zadatim id-jem.");
		}
	...
	public Filijala dodajFilijalu(Adresa adresaFilijale) throws NevalidniPodaciException {
		...
		AdministratorRentACar admin = (AdministratorRentACar) SecurityContextHolder.getContext().getAuthentication()
				.getPrincipal();
	...
	public Collection<Vozilo> pretraziVozilazaBrzuRezervaciju(PretragaVozilaDTO kriterijumiPretrage)
			throws NevalidniPodaciException {
		Optional<RentACarServis> pretragaRac = rentACarRepository.findById(kriterijumiPretrage.getIdRac());
	...
	public BrzaRezervacijaVozilaDTO dodajBrzuRezervaciju(BrzaRezervacijaVozilaDTO novaRezervacija)
			throws NevalidniPodaciException {
		...
		if (voziloJeNaBrzojRezervaciji(target, datumPreuzimanja, datumVracanja)) {
			throw new NevalidniPodaciException(
					"Dato vozilo se vec nalazi na brzoj rezervaciji u zadatom vremenskom periodu");
		}
	...
	public BrzaRezervacijaVozilaDTO zadajPopustBrzeRezervacije(BrzaRezervacijaVozilaDTO brzaRezervacija)
			throws NevalidniPodaciException {
		Optional<BrzaRezervacijaVozila> rezervacijaSearch = brzaRezervacijaVozilaRepository
				.findById(brzaRezervacija.getIdBrzeRezervacije());
	...
	public PrikazBrzeRezVozilaDTO vratiZaPrikaz(BrzaRezervacijaVozilaDTO brzaRezervacija)
			throws NevalidniPodaciException {
	...
	public Collection<PrikazBrzeRezVozilaDTO> vratiBrzeZaPrikaz(Long idServisa) throws NevalidniPodaciException {
		...
		for (Vozilo voz : vozilaServisa) {
			for (BrzaRezervacijaVozila brzaRezervacija : brzaRezervacijaVozilaRepository.findAllByVozilo(voz)) {
				...
				double cijenaSaPopustom = punaCijena - brzaRezervacija.getProcenatPopusta() * punaCijena / 100;
				brzeRezDTO.add(new PrikazBrzeRezVozilaDTO(brzaRezervacija.getId(), voz.getNaziv(),
						voz.getRentACar().getNaziv(), brzaRezervacija.getDatumPreuzimanjaVozila(),
	...
	public Collection<BrzaRezervacijaVozila> pretraziVozilaSaPopustom(PretragaVozilaDTO kriterijumiPretrage)
			throws NevalidniPodaciException {
		...
		for (Vozilo vozilo : svaVozila) {
			...
			if (voziloJeNaBrzojRezervaciji(vozilo, pocetniDatum, krajnjiDatum)
					&& !voziloJeRezervisano(vozilo, pocetniDatum, krajnjiDatum)) {
		...
		for (RezervacijaVozila rVozila : sveRezervacije) {
			if (pocetniDatum != null && krajnjiDatum != null) {
				if (pocetniDatum.compareTo(rVozila.getDatumPreuzimanjaVozila()) <= 0
						&& krajnjiDatum.compareTo(rVozila.getDatumPreuzimanjaVozila()) >= 0) {
	...
	public IzvjestajDTO dnevniIzvjestaj(Long idServisa, DatumiZaPrihodDTO datumiDto) throws NevalidniPodaciException {
